...
typedef struct {
    ...
    ngx_int_t                                              front;       // 队列中的第一个元素
    ngx_int_t                                              rear;        // 队列中最后一个元素
    ngx_int_t                                              len;         // 队列长度
} ngx_http_vhost_traffic_status_node_time_queue_t;                      // 访问同一个域名的多次请求，每一次请求的持续时间都会放入这个队列
...
typedef struct {
    ...
    ngx_atomic_t                                           stat_request_time_counter;   // 该请求的持续时间之和（会访问多次）
    ngx_msec_t                                             stat_request_time;           // 最近一次请求 还是 第一次请求 的 持续时间
    ngx_http_vhost_traffic_status_node_time_queue_t        stat_request_times;          // 队列，其中的元素是每次请求的持续时间
    ngx_http_vhost_traffic_status_node_histogram_bucket_t  stat_request_buckets;        // 每个桶都代表一个持续时间段，统计各个请求的持续时间分散在各个时间段中的个数
